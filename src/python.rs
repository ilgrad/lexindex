...
/// Ordered dictionary with the key stored for every id: exact `string ↔ rank`, 2.64 B/key on real
/// words.
...
impl PyDictIndex {
    ...
    /// 512 / 1024 gave 2.85 / 2.72 / 2.66 / 2.64 / 2.52 / 2.51 bytes per key on the dictionary,
    ...
    /// unrouted, at the default block, `id` took 19-31 % less time on twelve corpora of thirteen
    /// at a million keys and 13-22 % on six at ten million; `paths`, whose restarts tie, did not
    /// move.
    ///
