...
impl PerfectHashIndex {
    ...
    /// table from their serialised parts, the arena from where it is.
...
mod tests {
    ...
    fn load_mmap_verified_refuses_a_flipped_payload_byte_the_plain_mapping_takes() {
        let idx = PerfectHashIndex::build(["GET", "POST", "PUT", "DELETE"]).unwrap();
        let path = std::env::temp_dir().join(format!("lexindex_mmapv_{}.bmp", std::process::id()));
        idx.save(&path).unwrap();
        // SAFETY: this test owns the file and nothing writes to it while a map is alive.
        let mapped = unsafe { PerfectHashIndex::load_mmap_verified(&path) }.unwrap();
        ...
        assert!(unsafe { PerfectHashIndex::load_mmap(&path) }.is_ok());
        assert!(unsafe { PerfectHashIndex::load_mmap_verified(&path) }.is_err());
        assert!(PerfectHashIndex::load(&path).is_err());
        std::fs::remove_file(&path).ok();
    }
