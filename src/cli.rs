...
//! `Display`, and `--index auto` builds the index that ladder puts first. Everything is `run`, so
//! the argument parsing, the four subcommands and every error path are reachable from a unit test,
//! and two crates compile this file: the `lexindex` binary, whose `main` is three lines over `run`,
//! and the Python extension, whose `lexindex` command and `python -m lexindex` call the same `run`
//! — one program, whichever way it was installed.
...
fn cmd_dump(cmd: &Cmd, out: &mut dyn Write) -> Result<(), Fail> {
    ...
    match info.kind {
        ...
        // `BlobKind` is `#[non_exhaustive]`, which binds only outside the crate that defines it:
        // the binary needs this arm, and the Python extension, which compiles this file inside
        // that crate, sees the match as complete without it.
        #[cfg(feature = "mph")]
