...
**2.64 B/key** (`StringIndex` 5.95), `id` 333–335 ns against 256–270, `key_into` 194–196 against
`key`'s 256–260; 128 and 512 give 2.66 and 2.52 B/key at 311–313 and 369–374 ns, and 1024 gives
2.51 at 405–410. Against one level at the same size the reverse lookup halves and `id` does not move:
...
watches a C header. Within a number the ABI is append-only, in its symbols and in the
`LexindexKind` values: a newer library serves an older header, whose caller meets a kind it does not
name only for a blob its own library refused. Removing or changing either bumps the number, which
is a major release of the crate. The header is generated from `src/capi.rs` by `cbindgen` and CI
regenerates it to compare, so the two cannot drift.
