...
# Exact string <-> rank both ways, about 2.64 B/key: the sorted keys front-coded in blocks of 256,
...
shuffled order — `id_unchecked` answers in 14 to 78 ns and `id` at eight bits in 19 to 103 over
nineteen corpora of half a million to ten million keys, against 245 to 716 for the dictionary's own
`id` and 54 to 673 for XCDAT, the fastest trie measured
...
let words = DictIndex::build(["apple", "apricot", "banana"])?; // ordered, keys stored, ~2.64 B/key
...
(`key(id)`), `prefix` (a prefix or range query, priced as a `prefix_count`), `common_prefix` and
`longest_prefix`; `batch=N` is the keys an `ids_of` call holds. The weights are relative, so counts
