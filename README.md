...
<!-- table: compare bench/results/compare-2026-09-19-arz-999e933.json columns=prefix,range,fuzzy,reverse,exact,mmap -->
...
pays a **51 ns** call boundary, which is what the empty loop and call cost in this run, against 49
...
`marisa-trie`'s 232–233, and the larger block trades reverse-lookup latency for the bytes. **The two `DictIndex` rows predate two 4.0.0 changes — the microblock start and the dropped pruning pass — and are 0.6 % high: the same build now writes 2.64 and 2.52 bytes a key.** Sizes are a function of the keys and were re-measured directly. The table itself waits for a run whose call floor is back at the 47 ns this machine gives when nothing else is on it — the re-measurement above came from one that floored at 61, so its latency column is 15–20 % high for reasons that have nothing to do with this library, and a half-refreshed table is worse than a dated one. `marisa-trie` appears three times for the same reason it has tuning parameters: its
