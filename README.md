...
- **`DictIndex`** — an **ordered** dictionary with the key stored for every id: `string ↔ rank` both
  ...
  **3.52 bytes/key**, 41 % below `StringIndex`, `id` 314–337 ns against its 346–363, `key_into`
  ...
  count it (127 657)**. Blocks of 128 store **2.89 bytes/key, under `marisa-trie`**, for a slower
  reverse lookup. Exact queries, every id back to its key, small.
...
[`bench/results/compare-2026-09-11-arz-00857e2-dirty.json`](https://github.com/ilgrad/lexindex/blob/main/bench/results/compare-2026-09-11-arz-00857e2-dirty.json)
...
the bytes. [The benchmark notes](https://github.com/ilgrad/lexindex/blob/main/docs/benchmarks.md)
table the whole block-size curve and put the prefix queries head to head.</sub>
