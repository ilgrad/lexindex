...
- **Measured against PtrHash and PHast on a cool machine, with three faults in the comparison
  ...
  on in generation order now. The `ph` crate's PHast+ hashes with wyhash in the harness: its default
  is std's SipHash-1-3, which every earlier table timed, so its rows read 2–4× slower than the crate
  intends (PHast+ 8.8 → 3.5 ns at 1 M with the right hasher, its build 69.6 → 58.3). And the
  harness's lookup loops were inlined into one function beside each row's build, which cost lexindex
  0.7–3.6 ns a lookup and the other rows nothing measurable; each row's loop now stands alone. The
...
- **[`plan_file(path, needs, objective)`](https://docs.rs/lexindex/latest/lexindex/fn.plan_file.html)**:
  the ranking of a keys file, without ever holding it. The file is sorted externally — runs in
  memory, spilled beside it, merged back — and the merge is where the key count, the mean length
  and both shared-prefix distances are counted, **exactly** rather than from a sample; only the
  three constants no statistic gives still come from one, and that sample is drawn by hash, which
  does not need the corpus in hand. So this is not an approximation of `plan`: on a 925 MB,
  7 343 721-line path list the two print the **same ladder to the byte**, at **0.27 GB** of
  resident memory against 1.30 and in less wall time. `lexindex plan <file>` and `lexindex build
  --index auto` use it, so a ranked build now streams like a named one — where `--index auto` used
  to read the corpus in and say on stderr that pricing it meant holding it. Standard input still
  cannot be planned this way: it cannot be read twice.

- **[`plan_for(keys, needs, objective)`](https://docs.rs/lexindex/latest/lexindex/fn.plan_for.html)
  and [`Objective`](https://docs.rs/lexindex/latest/lexindex/enum.Objective.html)**: the ranking
  can be for something other than the blob size. `Memory` is what `plan` has always answered and
  stays the default — `plan` is exactly `plan_for(…, Objective::Memory)`, and its signature has not
...
- **`lexindex build --stream`**: the command line builds without holding the corpus. With a named
  ...
  rewound. `--index auto` streams as well, since `plan_file` prices a file without holding it
  either: the ladder comes off the streamed plan and its winner is built the same way.
...
- **The research frontier, measured.** `bench/frontier/` builds the C² paper's benchmark — C²-FST,
  ...
  keys. lexindex is on the size–latency front on all thirteen corpora and all six; `DictIndex` is
  the smallest structure on twelve of the thirteen at a million keys and five of the six at ten
  million, the fastest on `dna` at ten million, and builds before every compressed trie on every
  corpus. The one corpus that keeps a trie smaller is `numeric`, where `StringIndex` holds the whole
  corpus in a few hundred bytes; C²-MARISA reads faster than `DictIndex` on ten of the thirteen
  and is larger on all of them. Nothing of the competitors is
...
- **`DictIndex` trains a symbol table per shard of 65 536 keys.** One table for the whole index is
  ...
  `/usr/share` and of a million under `/home` are different languages, and one sampled budget had
  to cover both. A shard is a whole number of blocks, each shard's table is trained on 10 000
  pieces from that shard alone, the tables are trained in parallel, and the blob carries them in
  order after the header, which names the shard in blocks. A lookup picks its table from the block
  number it already has: one division, and no memory access it did not already make. `paths` at a
  million keys goes **15.923 → 14.673 bytes a key**, `titles-ru` 12.110 → 11.364, `idents`
  7.270 → 6.658, `titles-en` at ten million 7.886 → 7.652 and the dictionary 2.852 → 2.838; what
  rises is `domains` (5.008 → 5.066) and a `word.word` grid (2.47 → 2.50), corpora whose shards are
  all one language and now pay for a table each. The ladder over 32 / 64 / 128 / 256 / 512 / 1024
  keys a block becomes 3.23 / 3.03 / 2.90 / 2.84 / 2.81 / 2.79 bytes a key. Training is linear in
  the sample, so the shard and the budget trade against each other at a fixed build cost: over eight
  corpora, shards of 262 144 / 131 072 / 65 536 / 32 768 / 16 384 keys with the budget halving
  alongside sit 1.5 / 0.8 / 0.4 / 0.4 / 0.9 % above the smallest size each corpus reaches, and
  65 536 is the cheap end of that flat bottom. **Lookups do not move**: A-B-A-B in one process on
